...
def draw_init():
    ...
    draw_text(screen, '← 鍵、→ 鍵-移動飛船，空白鍵-發射子彈', 24, WIDTH/2, HEIGHT/2)
...
while running:
    ...
    # 判斷石頭、飛船相撞
    # rockHits = pygame.sprite.spritecollide(player, rocks, False, pygame.sprite.collide_circle)
    rockHits = pygame.sprite.spritecollide(player, rocks, True, pygame.sprite.collide_circle)
    # if rockHits:
    #     running = False
    for hit in rockHits:
        new_rock()
    ...
    # 判斷寶物、飛船相撞
    treasureHits = pygame.sprite.spritecollide(player, powers, True)
    for hit in treasureHits:
        if hit.type == 'shield':
            player.health += 20
